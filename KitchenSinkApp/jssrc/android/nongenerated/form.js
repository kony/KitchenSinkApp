...
function frmOptions() {
    ...
    case 4:
        if (flag == 0) {
            flag = 1;
            if (kony.os.deviceInfo().name == "iPhone" || kony.os.deviceInfo().name == "iPad") kony.application.showLoadingScreen("loadingscreen", "Loading...", constants.LOADING_SCREEN_POSITION_ONLY_CENTER, true, true, {
                shouldShowLabelInBottom: true,
                separatorHeight: 30
            });
            else kony.application.showLoadingScreen("loadingscreen", "Loading...", constants.LOADING_SCREEN_POSITION_ONLY_CENTER, true, true, null);
 ...
 ******************************************************************/
function titleBar() {
    if (channel == "tablet" || channel == "desktopweb") frmTitlebarOptions.show();
    else frmTtlbarOptions.show();
}
/*****************************************************************
 *	Name    : goToHome
 *	Author  : Kony
