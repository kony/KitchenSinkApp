...
function handleResponse(pplObj) {
    ...
    for (var i = 0; i < pplObj["@records_count"]; i++) {
        ...
        else dummyObj.lblPName = frmAsyncDataAccess.tbxFName.text + " " + frmAsyncDataAccess.tbxLName.text;
...
function httpRequestCall() {
    if (flag == 0) {
        ...
        if (channel == "tablet" && kony.os.deviceInfo().name != "thinclient") frmDeviceFeatures.sbxDevFeatureDeatils.remove(frmAsyncDataAccess.hbxAsyncDataDisplay);
        flag = 1;
    ...
    try {
        ...
        if (channel == "tablet" || channel == "desktopweb") var apiKey = frmAsyncDataAccess.txtAPIKey.text;
        else var apiKey = frmAPIKey.txtAPIKey.text;
