...
function decrypt() {
    try {
        ...
        } else var myEncryptedTextRa = kony.convertToRawBytes(str.substring(17));
        var myClearText = kony.crypto.decrypt(algo, encryptDecryptKey, myEncryptedTextRa, prptobj);
        if (kony.os.deviceInfo().name == "WindowsPhone") frmCrypto.lblDecrypt.text = "Decrypted text = " + myClearText;
        else frmCrypto.lblDecrypt.text = "Decrypted text = " + myClearText.toString();
    } catch (err) {
        alert(typeof err);
...
function createHashMD2() {
    try {
        if (kony.os.deviceInfo().name == "WindowsPhone" || kony.os.deviceInfo().name == "Windows") var algo = "sha1";
        else var algo = "md2";
...
function createHashMD4() {
    try {
        if (kony.os.deviceInfo().name == "WindowsPhone" || kony.os.deviceInfo().name == "Windows") var algo = "sha256";
        else var algo = "md4";
