...
function rowTempSetData() {
    frmSegDynRowTemplate.segDynamicRowTemp.widgetDataMap = {
        lbl7: "lbl7",
        lbl8: "lbl8"
    }
    var data = [{
...
function collapseRowTemp() {
    var index = frmSegDynRowTemplate.segDynamicRowTemp.selectedIndex[1];
    if (kony.os.deviceInfo().name == "blackberry") alert("Trying to collapse the row at index " + index);
    var dataForRowTemp = {};
    if (index == 0) {
        dataForRowTemp = {
            lbl7: "Name",
