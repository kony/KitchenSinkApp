...
function insertFirstData(transactionID) {
    ...
    ((insertTable) != null) && i <= insertTable.length; i++) {
        ...
        var sqlStatement = "INSERT INTO emp_details VALUES (" + (count + 1000) + ",\"" + v[kony.decrement(1)] + "\"," + v[kony.decrement(2)] + ")";
        count = count + 1;
        kony.db.executeSql(transactionID, sqlStatement, null, success_insertData, commonErrorCallback);
    }
}
/*****************************************************************
...
function success_sqlSelect(transactionId, resultset) {
    if (resultset != null) {
        ...
        } else frmWebSQLResults.segWebSQLResults.setData(insertTable);
    }
    if (channel != "tablet" && channel != "desktopweb") frmWebSQLResults.show();
}
/*****************************************************************
 *	Name    : sqlSelect
 *	Author  : Kony
...
function sqlUpdate(transactionID) {
    ...
    kony.db.executeSql(transactionID, sqlStatement, null, success_sqlUpdate, commonErrorCallback);
}
/*****************************************************************
...
function sqlDelete(transactionID) {
    ...
    //Execute sql statement
    kony.db.executeSql(transactionID, sqlStatement, null, success_sqlDelete, commonErrorCallback);
}
/*****************************************************************
